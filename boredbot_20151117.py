...
def puts_notice(target, msg):
  for line in msg.split("\n"):
    puts_data("NOTICE %s :%s" % (target, line))
...
def mode2char(mode):
  if (mode in _server['modes']):
    return _server['modes'][mode]
  else:
    return ""
  
def char2mode(char):
  for mode in _server['modes']:
    if (_server['modes'][mode] == char):
      return mode
  else:
    return ""

def is_mode(mode):
  if (mode in _server['modes'].values()):
    return True
  else:
    return False
...
def gets_connected(server, botnick, network):
  ...
  puts_data("PROTOCTL NAMESX")
  puts_data("PROTOCTL UHNAMES")
...
def gets_names(chan, nicks):
  for item in nicks: # loop through the nicknames
    ...
    for char in nick:
      if (is_mode(char)):
        mode.append(char2mode(char))
        nick = nick[1:]

    _nicklist.add(nick, user, host)
    _nickchan.add(nick, chan)
    _nickchan.set_mode(nick, chan, mode)

    output("Found user on %s: %s (%s!%s) mode:(+%s)" % (chan, nick, user, host, mode))
...
def gets_ctcp(nick, user, host, text):
  ...
  if (lindex(text, 0) == "VERSION"):
    puts_ctcpr(nick, "VERSION %s v%s \"%s\" - damian (%s)" % (SYSTEM['name'], SYSTEM['version'], SYSTEM['phrase'], SYSTEM['date']))
...
def gets_chan_msg(nick, user, host, chan, text):
  ...
  if (text[:1] == INIT['trigger']):
    if not (lindex(text, 0)[1:] in cmd):
      ...
      t = threading.Thread(target = eval("user_%s" % lindex(text, 0)[1:]), args = (nick, user + "@" + host, chan, addslashes(lrange(text, 1, -1))))
      t.start()
...
def gets_join(nick, user, host, chan):
  ...
  # update the internal address -- for the moment we won't do this
...
def user_listchan(nick, uhost, chan, text):
  puts_msg(chan, "%s is on channels: %s" % (lindex(text, 0), _nickchan.get_chan(lindex(text, 0))))
  
def user_listnick(nick, uhost, chan, text):
  puts_msg(chan, "%s has these users: %s" % (lindex(text, 0), _nickchan.get_nick(lindex(text, 0))))

def user_tmp(nick, uhost, chan, text):
  puts_msg(chan, "sleeping for 10secs.")
  time.sleep(10)
  puts_msg(chan, "im awake!")
...
def connect():
  ...
  irc.connect((INIT['server'], INIT['port']))
  
  output("& Authenticating to server ..")
  puts_nick(INIT['nick'])
  puts_user(INIT['ident'], INIT['name'])
...
#IRCThread = threading.Thread(target = connect, args = ())
#IRCThread.start()
